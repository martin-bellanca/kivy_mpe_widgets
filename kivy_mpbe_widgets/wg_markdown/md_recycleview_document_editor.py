...
from kivy_mpbe_widgets.wg_markdown.md_recycleview_line_editors import DocLineDataDic, MDDocumentLineEditor
...
class MDDocumentEditor(FocusBehavior, ThemableBehavior, RecycleView,
    ...
    def unselect_data_item(self, data_item, anim=False):  # TODO: cambiar a unselect_from_data
        ...
        self._selected_indexs.remove(data_item['index'])
        # print(f'  Lista de Seleccionados despues{self._selected_indexs}')

        UnSelectItemEventDispatcher.do_something(self, data_item, data_item['index'])
    ...
    def activate_from_data(self, data_item, cursor_pos=None, anim=False):
        ...
        print(f'+++ Cursor= {data_item["cursor"]} ------------------------------')

        data_item['active'] = True
        data_item['mode_editor'] = self._mode_editor
        self._active_index = data_item['index']
    ...
    def unactivate(self, anim=False):
        ...
        if -1 < self._active_index < len(self.data):
            ...
            self.refresh_from_data()
            self._active_index = -1
            UnActivateItemEventDispatcher.do_something(self, data_item, data_item['index'])
    ...
    def on_touch_up(self, touch):  # Hay que ver como con data y data_items. Deberia tener actualizada la informacion de selccion en los dos
        ...
        if touch.grab_current is self:  # Verifica si este widget "grabó" el evento touch
            ...
            if touch.button == 'left':  #  Seleccion y Des-seleccion de los items (Boton Izquierdo)
                ...
                print(f'+++ Cursor desde touch= {cursor} -------------------------------')
