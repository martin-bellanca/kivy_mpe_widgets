...
class TestApp(App):
    ...
    def build(self):
        ...
        self.md_editor1 = MDDocumentLineEditor(mdline=ln, index=1)
        layout.add_widget(self.md_editor1)

        ln = MDLine(md_text='**Linea de Prueba:** asdkjfalskjdflaksdf', prev_line=None, next_line=None)
        self.md_editor2 = MDDocumentLineEditor(mdline=ln, index=2)
        layout.add_widget(self.md_editor2)

        # Boton 1 de prueba -------------------------------------
        btn1 = ClickButton(text_label='Cambiar el Texto', size_hint=(1, 0.5))
        btn1.flat = True
        layout.add_widget(btn1)
        btn1.bind(on_click=self.on_click_btn1)
        # Boton 2 de prueba -------------------------------------
        btn2 = ClickButton(text_label='Update Type', size_hint=(1, 0.5))
        btn2.flat = True
        layout.add_widget(btn2)
        btn2.bind(on_click=self.on_click_btn2)
        # Boton 3 de prueba -------------------------------------
        btn3 = ClickButton(text_label='Cambia el Numero de Linea', size_hint=(1, 0.5))
        btn3.flat = True
        layout.add_widget(btn3)
        btn3.bind(on_click=self.on_click_btn3)
    ...
    def on_click_btn3(self, touch, keycode, var):
        self.md_editor2.index = 154
